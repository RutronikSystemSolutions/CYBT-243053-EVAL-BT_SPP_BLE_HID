/******************************************************************************
* File Name:   le_hid_keyboard.c
*
* Description: Enable to use the CYBT-243053-EVAL as BT SPP and BLE HID device
*
...
/**
 * @def Enable or not the HCI mode
 *
 * In case it is enabled, you can use the HCI UART interface to control the device using a host mcu
 */
#undef USE_HCI_MODE
...
/**
 * Configuration variables defined in COMPONENT_btstack_v1/wiced_bt_cfg.c
 */
...
/**
 * @brief Store handle used for BT SPP connection
 *
 * If 0 -> no connected
 */
...
/**
 * @brief Store the number of bytes received over BT SPP
 */
uint32_t spp_rx_bytes = 0;

static uint8_t batt_level_emulated = 50;
...
static void hci_ble_hid_dev_handle_send_report( uint8_t type, uint8_t report_id, uint8_t *p_data, uint16_t length );
...
APPLICATION_START()
{
    ...
    // Check if NVRAM works or not
    wiced_bt_device_link_keys_t nvram_keys;
    if (app_read_nvram(SPP_NVRAM_ID, &nvram_keys, sizeof(wiced_bt_device_link_keys_t)) == 0)
    {
    	WICED_BT_TRACE("Cannot read NVRAM...\r\n");
    	nvram_keys.bd_addr[0] = 0;
    	nvram_keys.bd_addr[1] = 0;
    	if (app_write_nvram(SPP_NVRAM_ID, sizeof(wiced_bt_device_link_keys_t), &nvram_keys) == 0)
    	{
    		WICED_BT_TRACE("Cannot write NVRAM...\r\n");
    	}
    }
    else
    {
    	WICED_BT_TRACE("Read NVRAM success.\r\n");
    }
}


/**
 * @brief Initialize the BLE features (gatt database and advertisement
 *
 * This function is executed in the BTM_ENABLED_EVT management callback.
 */
void hci_ble_hid_dev_app_init(void)
{
    // Configure buttons available on the platform -> generate ISR
    wiced_platform_register_button_callback( WICED_PLATFORM_BUTTON_1,
    		hci_ble_hid_dev_interrupt_handler, NULL, WICED_PLATFORM_BUTTON_BOTH_EDGE);
...
/**
 * @brief Write a new value to the battery level characteristic
 */
static void ble_hid_battery_level(uint8_t new_level)
{
	uint16_t attr_handle = HDLC_BATTERY_SERVICE_BATTERY_LEVEL_VALUE;
	if ( hci_ble_hid_dev_state.paired && ( hci_ble_hid_dev_state.conn_id == 0 ) )
	{
		WICED_BT_TRACE("Paired, but no connection id...\r\n");
	}
	else
	{
		hci_ble_hid_dev_battery_service_battery_level[0] = new_level;
		wiced_bt_gatt_status_t gatt_status = wiced_bt_gatt_send_notification( hci_ble_hid_dev_state.conn_id, attr_handle, sizeof(uint8_t), hci_ble_hid_dev_battery_service_battery_level);
...
/**
 * @brief Interrupt handler called when the user press the button
 */
static void hci_ble_hid_dev_interrupt_handler( void* user_data, uint8_t pin )
{
	WICED_BT_TRACE("hci_ble_hid_dev_interrupt_handler\r\n");

	// BT SPP connected? -> send something over BT SPP (test)
	if (spp_handle != 0)
	{
		uint8_t test[5] = {0};
		test[0] = 'r';
		...
		test[3] = '\n';
		test[4] = '\0';
		WICED_BT_TRACE("Send over BT SPP\r\n");
		wiced_bt_spp_send_session_data(spp_handle, test, 5);
	}

	// BLE HID connected? -> send something (test)
	if (hci_ble_hid_dev_state.conn_id != 0)
	{
		// Size: HCI_BLE_HID_DEV_INPUT_REPORT_SIZE = 8
		uint8_t record_data_a[] = {0, 0, 4, 0, 0, 0, 0, 0}; // -> 'a'
		uint8_t record_data_release[] = {0, 0, 0, 0, 0, 0, 0, 0}; // -> key released
		wiced_trace_array(record_data_a, sizeof(record_data_a));

		WICED_BT_TRACE("Send 'a' over BLE HID\r\n");
		hci_ble_hid_dev_handle_send_report(HID_REPORT_TYPE_INPUT, HID_INPUT_REPORT_ID, record_data_a, ( uint16_t ) sizeof(record_data_a));

		WICED_BT_TRACE("Send released over BLE HID\r\n");
		hci_ble_hid_dev_handle_send_report(HID_REPORT_TYPE_INPUT, HID_INPUT_REPORT_ID, record_data_release, ( uint16_t ) sizeof(record_data_release));

		batt_level_emulated++;
		ble_hid_battery_level(batt_level_emulated);
	}

}

static uint8_t BLE_LOCAL_NAME[] = { 'b','l', 'e', '_', 'h', 'i', 'd','\0' };

/**
 * @brief Set BLE advertisement data
 */
static void hci_ble_hid_dev_set_advertisement_data( void )
{
	...
	adv_elem[num_elem].len          = strlen( (char *)BLE_LOCAL_NAME);
	adv_elem[num_elem].p_data       = ( uint8_t* )BLE_LOCAL_NAME;
    num_elem++;
...
/**
 * @brief Prepare extended inquiry response data.  Current version publishes device name and 16bit.
 * This is for BT SPP service
 */
void app_write_eir(void)
{
    ...
    length = strlen((char *)hci_ble_hid_cfg_settings.device_name);

    *p++ = length + 1;
    *p++ = BT_EIR_COMPLETE_LOCAL_NAME_TYPE;        // EIR type full name
    memcpy(p, hci_ble_hid_cfg_settings.device_name, length);
    p += length;
...
/**
 * @brief Callback function called when new connection over BT SPP
 */
...
/**
 * @brief Callback function called when connection over BT SPP is down
 */
...
/**
 * @brief Callback function called to signal that new data are available from the BT SPP
 *
 * i.e. The host sent data to the BT module over BT SPP
 */
wiced_bool_t spp_rx_data_callback(uint16_t handle, uint8_t* p_data, uint32_t data_len)
{
	WICED_BT_TRACE("%s handle:%d len:%d data[0]:%02x, total rx %d\r\n", __FUNCTION__, handle, data_len, p_data[0], spp_rx_bytes);

    spp_rx_bytes += data_len;

    uint8_t test[8] = {0};
    for(uint32_t i = 0; (i < data_len) && (i < 5); ++i)
    {
    	test[i] = p_data[i];
    }

    int index = (data_len < 5) ? data_len: 5;
    test[index] = 'o';
    test[index+1] = 'k';
    test[index+2] = '\0';

    // Send something back (as test)
    return wiced_bt_spp_send_session_data(handle, test, index + 3);
    //    return WICED_TRUE;
}
...
/**
 * @brief Called when data are available over the PUART interfance
 */
void puart_rx_interrupt_callback(void* unused)
{
	WICED_BT_TRACE("puart_rx_interrupt_callback\r\n");
	uint8_t buffer[16] = {0};
	uint8_t  readbyte = 0;
	int offset = 0;
	for(int i = 0; i < 16; ++i)
	{
		if (!wiced_hal_puart_rx_fifo_not_empty()) break;
		if (!wiced_hal_puart_read(&buffer[offset])) break;
		offset++;
	}
	WICED_BT_TRACE("read: %d - first = %d \r\n", offset, buffer[0]);


	// In case BT SPP is connected, send it over COM port
	if (spp_handle != 0)
	{
		wiced_bt_spp_send_session_data(spp_handle, buffer, offset);
	}

	// Remark:
	// we do not send information about BLE HID to avoid a "loop" (since it will "emulate" a keyboard touch...)

	if (offset != 16)
	{
		// If we read everything - clear interrupt
		wiced_hal_puart_reset_puart_interrupt( );
	}
}

/**
 * @brief Initialize BT SPP
 */
static void app_spp_init()
{
	app_write_eir();
    ...
    wiced_bt_dev_set_discoverability(BTM_GENERAL_DISCOVERABLE,
    		hci_ble_hid_cfg_settings.br_edr_scan_cfg.inquiry_scan_window,
			hci_ble_hid_cfg_settings.br_edr_scan_cfg.inquiry_scan_interval);

    wiced_bt_dev_set_connectability(BTM_CONNECTABLE,
    		hci_ble_hid_cfg_settings.br_edr_scan_cfg.page_scan_window,
			hci_ble_hid_cfg_settings.br_edr_scan_cfg.page_scan_interval);

    // Some stuff for PUART
...
/**
 * @brief Enable pairing over BLE
 */
static void enable_ble_hid_pairing()
{
    WICED_BT_TRACE( "enable_ble_hid_pairing %d\r\n");
    wiced_bt_set_pairable_mode(WICED_TRUE, 0);
...
/**
 * @brief Most important function: Will be called by the BT/BLE stack when something happens
 *
 * It check which event has been generated and does what necessary
 *
 * @param [in] event Type of event (processed using switch case)
 * @param [in] p_event_data Data corresponding to the event
 */
static wiced_bt_dev_status_t hci_ble_hid_dev_management_callback( wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data )
{
    ...
    case BTM_ENABLED_EVT:
        ...
        if (WICED_BT_SUCCESS == p_event_data->enabled.status)
        {
            ...
            // First initialize BLE HID (Gatt database, and enable pairing - advertisement)
            hci_ble_hid_dev_app_init();
            enable_ble_hid_pairing();
            // Then initialize BT SPP profile
            app_spp_init();
        }
        break;
    ...
    case BTM_USER_CONFIRMATION_REQUEST_EVT:
        ...
        status = wiced_bt_dev_get_remote_name(p_event_data->user_confirmation_request.bd_addr, spp_bt_remote_name_callback);
        break;
    ...
    case BTM_PAIRING_COMPLETE_EVT:
    	...
    	wiced_bt_dev_pairing_cplt_t* pairing_info = &p_event_data->pairing_complete;
    	// Check if BR/EDR or BLE
    	if (pairing_info->transport == BT_TRANSPORT_BR_EDR)
    	{
    		WICED_BT_TRACE("==> BR EDR\r\n");
    		WICED_BT_TRACE("Pairing Complete: %d\r\n", pairing_info->pairing_complete_info.br_edr.status);
    	...
    	else
    	{
			...
			if ( p_info->reason == WICED_SUCCESS )
			{
				...
				if (wiced_bt_ble_update_advertising_filter_accept_list( WICED_TRUE, p_event_data->pairing_complete.bd_addr ) != WICED_TRUE)
				{
    ...
    case BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT:
    	...
    	// Receive new device link keys -> need to store them to non volatile memory
    	if (memcmp(p_event_data->paired_device_link_keys_update.bd_addr, null_bdaddr, BD_ADDR_LEN) != 0)
    	{
    		...
    		// Save it to RAM using memcpy and to non-volatile memory using app_write_nvram
    ...
    case BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT:
    	...
    	// The BT/BLE stack requests from us the device link keys
    	// we need to read them from the NVRAM and store them to p_event_data->paired_device_link_keys_request.key_data
    	wiced_bt_device_link_keys_t nvram_keys;
    	if (app_read_nvram(SPP_NVRAM_ID, &nvram_keys, sizeof(wiced_bt_device_link_keys_t)) != 0)
		{
			status = WICED_BT_SUCCESS;
			WICED_BT_TRACE("NVRAM address :%B\r\n", nvram_keys.bd_addr);
			WICED_BT_TRACE("pevent address :%B\r\n", p_event_data->paired_device_link_keys_request.bd_addr);

			if ( memcmp( p_event_data->paired_device_link_keys_request.bd_addr,
					nvram_keys.bd_addr, BD_ADDR_LEN ) == 0 )
			{
				memcpy( &p_event_data->paired_device_link_keys_request.key_data,
						&nvram_keys.key_data, sizeof(wiced_bt_device_sec_keys_t) );
				WICED_BT_TRACE("Key retrieval success\r\n");
			}
			else
			{
				status = WICED_BT_ERROR;
				WICED_BT_TRACE("Key retrieval failure - ADDR does not match.\r\n");
			}
		...
		{
			status = WICED_BT_ERROR;
			WICED_BT_TRACE("app_read_nvram Key retrieval failure\r\n");
		}
        break;
    ...
    case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
    	...
    	wiced_bt_ble_advert_mode_t previous_adv_mode = hci_ble_hid_dev_state.adv_mode;
    ...
    case BTM_BLE_CONNECTION_PARAM_UPDATE:
    	wiced_bt_ble_connection_param_update_t *p_ble_conn_param_update = &p_event_data->ble_connection_param_update;
        WICED_BT_TRACE ("BTM_BLE_CONNECTION_PARAM_UPDATE event status:%d interval:%d latency:%d lsto:%d\r\n",
...
//            store_nvram = WICED_TRUE;
    ...
    // TODO: reconsider to store configuration?
//    if ( store_nvram )
//    {
...
//    {
//    	WICED_BT_TRACE("Start advertisement.\r\n");
//        // start advertisements so that a host can connect
//        wiced_bt_start_advertisements( BTM_BLE_ADVERT_UNDIRECTED_HIGH, 0, NULL );
//    }
    wiced_bt_start_advertisements( BTM_BLE_ADVERT_UNDIRECTED_HIGH, 0, NULL );
...
#ifdef USE_HCI_MODE
    // propagate event to host (upper layer application)
 ...
 * Send HID report
 *
 * @param [in] type: Should be a HID_REPORT_TYPE_INPUT
 * @param [in] report_id: In our case should be HID_INPUT_REPORT_ID or HID_INPUT_REPORT2_ID
 * @param [in] p_data: Pointer to the report values
 * @param [in] length: Length of the buffer pointed by p_data
 */
void hci_ble_hid_dev_handle_send_report( uint8_t type, uint8_t report_id, uint8_t *p_data, uint16_t length )
{
	// TODO store it in NVRAM?
...
//                client_configuration_descriptor = hci_ble_hid_dev_state.host_info.input_report2_client_configuration;
    ...
    {
    	// TODO: check for client_configuration_descriptor as below!!!!
    	// Remark needs to be save in NVRAM (for reboot case) and read in the state BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT
...
//    else
//    {
//    	WICED_BT_TRACE("client_configuration_descriptor is %d\r\n", client_configuration_descriptor);
//    }
}
...
void hci_control_le_send_connect_event( uint8_t addr_type, BD_ADDR addr, uint16_t con_handle, uint8_t role )
{
	WICED_BT_TRACE("hci_control_le_send_connect_event\r\n");
...
{
	WICED_BT_TRACE("hci_ble_hid_dev_handle_command - length = %d - First = %d \r\n", (int)length, p_data[0]);
...
void hci_ble_hid_dev_send_data( uint8_t report_type, uint8_t report_id, uint8_t *p_data, uint16_t len )
{
    ...
    wiced_transport_send_data( HCI_CONTROL_HIDD_EVENT_DATA, tx_buf, ( int ) ( len < 200 ? 2 + len : 200 ) );
}
...
void hci_ble_hid_handle_command( uint16_t cmd_opcode, uint8_t* p_data, uint32_t data_len )
{
    ...
    case HCI_CONTROL_HIDD_COMMAND_PUSH_PAIRING_HOST_INFO:
    ...
    case HCI_CONTROL_HIDD_COMMAND_SEND_REPORT:
        ...
        hci_ble_hid_dev_handle_send_report(p_data[1], p_data[2], &p_data[3], ( uint16_t ) ( data_len - 3 ));
