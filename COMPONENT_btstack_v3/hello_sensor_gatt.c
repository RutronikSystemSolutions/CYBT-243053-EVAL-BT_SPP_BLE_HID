...
#endif
#include "wiced_timer.h"
#include "le_hid_keyboard.h"

/* Extern functions */
void                     le_hid_keyboard_led_blink(uint16_t on_ms, uint16_t off_ms, uint8_t num_of_blinks );
attribute_t * le_hid_keyboard_get_attribute( uint16_t handle );
void le_hid_keyboard_send_message( void );
wiced_bt_gatt_status_t le_hid_keyboard_gatts_conn_status_cb( wiced_bt_gatt_connection_status_t *p_status );
wiced_bt_gatt_status_t le_hid_keyboard_gatts_req_cb( wiced_bt_gatt_attribute_request_t *p_req );
wiced_bt_gatt_status_t le_hid_keyboard_gatts_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data);
wiced_bt_gatt_status_t le_hid_keyboard_gatt_op_comp_cb( wiced_bt_gatt_operation_complete_t *p_data );

/* Extern variables */
extern wiced_bt_cfg_ble_t wiced_bt_cfg_ble;
extern le_hid_keyboard_state_t le_hid_keyboard_state;
extern host_info_t le_hid_keyboard_hostinfo;
extern wiced_timer_t le_hid_keyboard_conn_idle_timer;

uint8_t *le_hid_keyboard_alloc_buffer(uint16_t len);
void le_hid_keyboard_free_buffer(uint8_t *p_data);

/*
 * helper function to init GATT
 *
 */
void le_hid_keyboard_gatt_init()
{
    wiced_bt_gatt_status_t gatt_status;
    /* Register with stack to receive GATT callback */
    gatt_status = wiced_bt_gatt_register(le_hid_keyboard_gatts_callback);
    WICED_BT_TRACE( "wiced_bt_gatt_register: %d\n", gatt_status );

    /*  Tell stack to use our GATT database */
    gatt_status =  wiced_bt_gatt_db_init( le_hid_keyboard_gatt_database, le_hid_keyboard_gatt_database_size, NULL );
...
wiced_bt_gatt_status_t le_hid_keyboard_gatts_callback( wiced_bt_gatt_evt_t event, wiced_bt_gatt_event_data_t *p_data)
{
    ...
    case GATT_CONNECTION_STATUS_EVT:
        result = le_hid_keyboard_gatts_conn_status_cb( &p_data->connection_status );
        break;

    case GATT_ATTRIBUTE_REQUEST_EVT:
        result = le_hid_keyboard_gatts_req_cb( &p_data->attribute_request );
        break;

   case GATT_GET_RESPONSE_BUFFER_EVT:
        p_data->buffer_request.buffer.p_app_rsp_buffer = le_hid_keyboard_alloc_buffer (p_data->buffer_request.len_requested);
        p_data->buffer_request.buffer.p_app_ctxt       = le_hid_keyboard_free_buffer;
    ...
    case GATT_OPERATION_CPLT_EVT:
        result = le_hid_keyboard_gatt_op_comp_cb( &p_data->operation_complete );
        break;
    default:
        break;
    }
 ...
 */
wiced_bt_gatt_status_t le_hid_keyboard_gatts_req_read_handler (uint16_t conn_id, wiced_bt_gatt_opcode_t opcode, wiced_bt_gatt_read_t *p_read_req, uint16_t len_requested)
{
    attribute_t *puAttribute;
    uint16_t    attr_len_to_copy;
    uint8_t     *from;

    if ( ( puAttribute = le_hid_keyboard_get_attribute (p_read_req->handle) ) == NULL)
    {
    ...
    {
        if (le_hid_keyboard_state.battery_level++ > 5)
            le_hid_keyboard_state.battery_level = 0;
    }
 ...
 */
wiced_bt_gatt_status_t le_hid_keyboard_gatts_req_read_by_type_handler (uint16_t conn_id, wiced_bt_gatt_opcode_t opcode,
    wiced_bt_gatt_read_by_type_t *p_read_req, uint16_t len_requested)
{
    attribute_t *puAttribute;
    uint16_t    attr_handle = p_read_req->s_handle;
    uint8_t     *p_rsp = le_hid_keyboard_alloc_buffer (len_requested);
    uint8_t    pair_len = 0;
    int used = 0;
    ...
    while (WICED_TRUE)
    {
        ...
        if ((puAttribute = le_hid_keyboard_get_attribute (attr_handle)) == NULL)
        {
            ...
            le_hid_keyboard_free_buffer(p_rsp);
            return WICED_BT_GATT_INVALID_HANDLE;
        }
        // --------
    ...
    /* Send the response */
    wiced_bt_gatt_server_send_read_by_type_rsp (conn_id, opcode, pair_len, used, p_rsp, le_hid_keyboard_free_buffer);
 ...
 */
wiced_bt_gatt_status_t le_hid_keyboard_gatts_req_read_multi_handler (uint16_t conn_id, wiced_bt_gatt_opcode_t opcode,
    wiced_bt_gatt_read_multiple_req_t *p_read_req, uint16_t len_requested)
{
    ...
    for (xx = 0; xx < p_read_req->num_handles; xx++)
    {
        ...
        if ((puAttribute = le_hid_keyboard_get_attribute (handle)) == NULL)
        {
    ...
    /* Send the response */
    wiced_bt_gatt_server_send_read_multiple_rsp (conn_id, opcode, used, p_rsp, le_hid_keyboard_free_buffer);
 ...
 */
wiced_bt_gatt_status_t le_hid_keyboard_gatts_req_write_handler( uint16_t conn_id, wiced_bt_gatt_opcode_t opcode, wiced_bt_gatt_write_req_t* p_data )
{
    ...
    case HANDLE_HSENS_SERVICE_CHAR_CFG_DESC:
        ...
        le_hid_keyboard_hostinfo.characteristic_client_configuration = p_attr[0] | ( p_attr[1] << 8 );

        WICED_BT_TRACE ("[%s] CCCD changed to: 0x%04x\n ", __FUNCTION__, le_hid_keyboard_hostinfo.characteristic_client_configuration);
    ...
    case HANDLE_HSENS_SERVICE_CHAR_BLINK_VAL:
        ...
        le_hid_keyboard_hostinfo.number_of_blinks = p_attr[0];
        if ( le_hid_keyboard_hostinfo.number_of_blinks != 0 )
        {
            WICED_BT_TRACE( "[%s] num blinks: %d\n", __FUNCTION__, le_hid_keyboard_hostinfo.number_of_blinks );
            nv_update = WICED_TRUE;
            le_hid_keyboard_led_blink (250,250,le_hid_keyboard_hostinfo.number_of_blinks);
        }
        break;
    ...
    if ( nv_update )
    {
        ...
        wiced_hal_write_nvram(le_hid_keyboard_VS_ID, sizeof(le_hid_keyboard_hostinfo),
            (uint8_t*)&le_hid_keyboard_hostinfo, &status);
 ...
 */
wiced_bt_gatt_status_t le_hid_keyboard_gatts_req_write_exec_handler( uint16_t conn_id, wiced_bt_gatt_exec_flag_t exec_falg)
{
 ...
 */
wiced_bt_gatt_status_t le_hid_keyboard_gatts_req_mtu_handler( uint16_t conn_id, uint16_t mtu)
{
 ...
 */
wiced_bt_gatt_status_t le_hid_keyboard_gatts_req_conf_handler( uint16_t conn_id, uint16_t handle )
{
    WICED_BT_TRACE( "le_hid_keyboard_indication_cfm, conn %d hdl %d\n", conn_id, handle );

    if ( !le_hid_keyboard_state.flag_indication_sent )
    {
        WICED_BT_TRACE("Hello: Wrong Confirmation!");
        return WICED_BT_GATT_SUCCESS;
    }

    le_hid_keyboard_state.flag_indication_sent = 0;

    /* We might need to send more indications */
    if ( le_hid_keyboard_state.num_to_write )
    {
        le_hid_keyboard_state.num_to_write--;
        le_hid_keyboard_send_message();
    }
    /* if we sent all messages, start connection idle timer to disconnect */
    if ( !le_hid_keyboard_state.flag_stay_connected && !le_hid_keyboard_state.flag_indication_sent )
    {
        if (wiced_is_timer_in_use(&le_hid_keyboard_conn_idle_timer) )
        {
            wiced_stop_timer(&le_hid_keyboard_conn_idle_timer);
            wiced_start_timer(&le_hid_keyboard_conn_idle_timer, le_hid_keyboard_CONN_IDLE_TIMEOUT_IN_SECONDS);
        }
    }
 ...
 */
wiced_bt_gatt_status_t le_hid_keyboard_gatts_req_cb( wiced_bt_gatt_attribute_request_t *p_data )
{
    wiced_bt_gatt_status_t result = WICED_BT_GATT_INVALID_PDU;

    WICED_BT_TRACE( "le_hid_keyboard_gatts_req_cb. conn %d, opcode %d\n", p_data->conn_id, p_data->opcode);

    switch (p_data->opcode)
    {
    case GATT_REQ_READ:
    case GATT_REQ_READ_BLOB:
        result = le_hid_keyboard_gatts_req_read_handler( p_data->conn_id, p_data->opcode, &p_data->data.read_req, p_data->len_requested );
        break;

    case GATT_REQ_READ_BY_TYPE:
        result = le_hid_keyboard_gatts_req_read_by_type_handler (p_data->conn_id, p_data->opcode, &p_data->data.read_by_type, p_data->len_requested);
        break;

    case GATT_REQ_READ_MULTI:
    case GATT_REQ_READ_MULTI_VAR_LENGTH:
        result = le_hid_keyboard_gatts_req_read_multi_handler (p_data->conn_id, p_data->opcode, &p_data->data.read_multiple_req, p_data->len_requested);
        break;

    case GATT_REQ_WRITE:
    case GATT_CMD_WRITE:
    case GATT_CMD_SIGNED_WRITE:
        result = le_hid_keyboard_gatts_req_write_handler( p_data->conn_id, p_data->opcode, &(p_data->data.write_req) );
    ...
    case GATT_REQ_EXECUTE_WRITE:
        le_hid_keyboard_gatts_req_write_exec_handler(p_data->conn_id, p_data->data.exec_write_req.exec_write);
        wiced_bt_gatt_server_send_execute_write_rsp(p_data->conn_id, p_data->opcode);
        break;

    case GATT_REQ_MTU:
        result = le_hid_keyboard_gatts_req_mtu_handler( p_data->conn_id, p_data->data.remote_mtu);
        break;

    case GATT_HANDLE_VALUE_CONF:
        result = le_hid_keyboard_gatts_req_conf_handler( p_data->conn_id, p_data->data.confirm.handle );
        break;
...
/*
 * GATT operation started by the keyboard has been completed
 */
wiced_bt_gatt_status_t le_hid_keyboard_gatt_op_comp_cb( wiced_bt_gatt_operation_complete_t *p_data )
{
    wiced_result_t              status;

    WICED_BT_TRACE("==>le_hid_keyboard conn %d op %d st %d\n", p_data->conn_id, p_data->op, p_data->status );
